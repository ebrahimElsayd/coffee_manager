...
export class SupabaseManagerDataSource implements ManagerDataSource {
 ...
 const db=getSupabaseBrowserClient();
  ...
  return orders.map(o=>{const items=itemsByOrder.get(String(o.id))??[];const drinks=items.map((i:any)=>{const product=Array.isArray(i.menu_products)?i.menu_products[0]:i.menu_products;return {id:i.id,productId:i.product_id,name:i.product_name??"",imageUrl:product?.image_url??undefined,recipientName:i.recipient_name??"Guest",quantity:i.quantity,unitPrice:Number(i.unit_price||0),note:i.notes??"",selectedOptions:normalizeSelectedOptions(i.selected_options),status:drinkStatus[i.status]??"New"};});const people=[...new Set(drinks.map((d:any)=>d.recipientName).filter(Boolean))];const isOrderPaid=o.payment_status==="paid";const sessionPayments=payments.filter((p:any)=>p.session_id===o.session_id);const payment=isOrderPaid?(sessionPayments.find((p:any)=>Array.isArray(p.session_receipts)?p.session_receipts.length>0:Boolean(p.session_receipts))??sessionPayments[0]):undefined;const receipt=Array.isArray(payment?.session_receipts)?payment.session_receipts[0]:payment?.session_receipts;const received=payment?.received_amount ?? payment?.received;const session=Array.isArray(o.table_sessions)?o.table_sessions[0]:o.table_sessions;const table=Array.isArray(session?.cafe_tables)?session.cafe_tables[0]:session?.cafe_tables;return{id:String(o.id),sessionId:String(o.session_id),orderNumber:Number(o.order_number),table:table?.table_number==null?"":String(table.table_number),customer:people[0]??"Guest",time:new Date(o.created_at).toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"}),openedAt:o.created_at,guests:people.length?`${people.length} Guests`:"",status:orderStatus[o.status]??"New",drinks,orderType:"DineIn",paymentStatus:isOrderPaid?"Paid":"Unpaid",paymentMethod:payment?.method?String(payment.method).replace(/^./,(c:string)=>c.toUpperCase()) as PaymentMethod:undefined,cashReceived:received==null?undefined:Number(received),cashChange:payment?.change_amount==null?undefined:Number(payment.change_amount),receiptNumber:receipt?.receipt_number==null?undefined:Number(receipt.receipt_number),receiptIssuedAt:receipt?.issued_at??undefined,sessionStatus:session?.status==="closed"?"Closed":"Open",closedAt:session?.closed_at??o.closed_at??undefined} as ManagerOrder;});
 }
