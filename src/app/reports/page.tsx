...
export default function ReportsPage() {
  ...
  const [recentOrderLimit, setRecentOrderLimit] = useState(10);
  useEffect(() => {
    let active = true;
    const refresh = () => { void Promise.all([getReportsSnapshot(range), getRecentPaidOrders(range, recentOrderLimit)]).then(([value, orders]) => { if (active) { setReport(value); setRecentOrders(orders); } }).catch(() => { if (active) { setReport(emptyReport); setRecentOrders([]); } }); };
    refresh();
  ...
  return <main className="manager-page min-h-screen overflow-x-hidden bg-[#080a09] pl-16 text-[#f4efe5] lg:pl-[184px] xl:pl-[200px] 2xl:pl-[224px]">
    ...
    <div className="min-h-screen p-4 sm:p-6 xl:p-8">
      ...
      <section className="mt-5 rounded-2xl border border-white/15 bg-[#111312] p-5 sm:p-6">
        <div className="flex items-start justify-between gap-3"><div><h2 className="font-serif text-xl">{pick("Recent paid orders", "أحدث الطلبات المدفوعة")}</h2><p className="mt-1 text-xs text-white/40">{pick(`Last ${recentOrderLimit} orders in the selected period`, `آخر ${recentOrderLimit} طلبات خلال الفترة المحددة`)}</p></div><div className="flex items-center gap-3">{recentOrderLimit === 10 && <button type="button" onClick={() => setRecentOrderLimit(20)} className="rounded-lg border border-[var(--gold)]/55 bg-[var(--gold)]/[.08] px-3 py-1.5 text-sm font-medium text-[#f5ca72] transition hover:bg-[var(--gold)]/[.16]">{pick("More", "المزيد")} <span aria-hidden="true">›</span></button>}<span className="text-xs text-white/40">{recentOrders.length}/{recentOrderLimit}</span></div></div>
