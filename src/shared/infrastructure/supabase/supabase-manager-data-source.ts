...
export class SupabaseManagerDataSource implements ManagerDataSource {
 ...
 async listTables(){const db=getSupabaseBrowserClient();if(!db)return[];const cafeId=await getManagerCafeId();const {data,error}=await db.from("cafe_tables").select("id,table_number,status").eq("cafe_id",cafeId).order("table_number");if(error)throw error;return(data??[]).map((t:any)=>({number:String(t.table_number),guests:"",status:(t.status==="occupied"?"New":"Available") as ManagerTable["status"],sessionStatus:(t.status==="occupied"?"Open":"Closed") as TableSessionStatus}));}
 async updateTableSession(table:string,status:TableSessionStatus){const db=getSupabaseBrowserClient();if(!db)throw new Error("Supabase unavailable");const cafeId=await getManagerCafeId();if(status!=="Closed"){const {error}=await db.from("cafe_tables").update({status:"occupied"}).eq("cafe_id",cafeId).eq("table_number",Number(table));if(error)throw error;return;}const {error}=await db.rpc("manager_close_table_session",{p_cafe_id:cafeId,p_table_number:Number(table)});if(error)throw error;}
}
