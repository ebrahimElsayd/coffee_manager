...
export type BillBranding = { cafeName?: string; branchName?: string; phone?: string; address?: string; currency?: string; footer?: string; locale?: "ar" | "en"; receipt?: { header?: string; footer?: string; showPrices: boolean; serviceEnabled: boolean; serviceType: "percent" | "fixed"; serviceValue: number; taxEnabled: boolean; taxType: "percent" | "fixed"; taxValue: number } };
...
export function printBill(table: string, orders: ManagerOrder[], branding: BillBranding = {}, cash?: { received: number; change: number }, targetWindow?: Window | null, autoPrint = false, receipt?: ReceiptMetadata) {
  ...
  const labels = branding.locale === "ar" ? {
    table: "الطاولة", item: "الصنف", quantity: "الكمية", price: "السعر", total: "الإجمالي", payment: "الدفع",
    paidCash: "مدفوع · نقدي", unpaidBill: "فاتورة غير محصلة", service: "الخدمة", tax: "الضريبة",
    cashReceived: "المبلغ المستلم", change: "الباقي", printReceipt: "طباعة الإيصال", printBill: "طباعة الفاتورة",
    cafeManagement: "إدارة الكافيه", preparedWithCare: "فاتورة الطاولة · أُعدت بعناية",
  } : {
    table: "TABLE", item: "Item", quantity: "Qty", price: "Price", total: "Total", payment: "Payment",
    paidCash: "PAID · CASH", unpaidBill: "UNPAID · BILL", service: "Service charge", tax: "Tax",
    cashReceived: "Cash received", change: "Change", printReceipt: "Print Receipt", printBill: "Print Bill",
    cafeManagement: "CAFE MANAGEMENT", preparedWithCare: "Modern table receipt · prepared with care",
  };
  const documentLanguage = branding.locale === "ar" ? "ar" : "en";
  const documentDirection = branding.locale === "ar" ? "rtl" : "ltr";
  const issued = formatReceiptDateTime(receipt?.issuedAt ?? new Date().toISOString(), branding.locale);
  const receiptLabel = receipt ? `${branding.locale === "ar" ? "إيصال رقم" : "Receipt No."} #${receipt.number}` : (branding.locale === "ar" ? "فاتورة غير محصلة" : "Unpaid bill");
  const orderNumbers = [...new Set(orders.map((order) => order.orderNumber).filter((number): number is number => Number.isFinite(number)))];
  const orderLabel = orderNumbers.length
    ? `${branding.locale === "ar" ? "طلب" : "Order"} ${orderNumbers.map((number) => `#${number}`).join(", ")}`
    : "";
  ...
  if (!bill) {
    ...
    frame.srcdoc = `<!doctype html><html lang="${documentLanguage}" dir="${documentDirection}"><head><title>${cafeName} · ${labels.table} ${table}</title><style>body{font:14px Arial;padding:24px;color:#222}h1{margin:0 0 8px}h2{margin:0 0 18px;color:#8b5d19}h3{margin:16px 0 6px;border-top:1px solid #ddd;padding-top:12px}p{margin:5px 0}.total{margin-top:20px;border-top:2px solid #222;padding-top:12px;font-size:18px;font-weight:bold}</style></head><body><h1>${cafeName}</h1><h2>${escapeHtml(receiptLabel)} · ${labels.table} ${table}</h2><p>${escapeHtml(orderLabel)}</p><p>${issued.date} · ${issued.time}</p>${fallbackRows}${branding.receipt?.serviceEnabled ? `<p>${labels.service}: ${service.toFixed(2)} ${currency}</p>` : ""}${branding.receipt?.taxEnabled ? `<p>${labels.tax}: ${tax.toFixed(2)} ${currency}</p>` : ""}<p class="total">${labels.total}: ${total.toFixed(2)} ${currency}</p></body></html>`;
  ...
  bill.document.open();
  bill.document.write(`<!doctype html><html lang="${documentLanguage}" dir="${documentDirection}"><head><title>${cafeName} · ${labels.table} ${table}</title><style>*{box-sizing:border-box}body{margin:0;background:#ece8df;color:#242019;font-family:Arial,sans-serif;padding:28px}.receipt{max-width:440px;margin:auto;overflow:hidden;background:#fff;border-radius:20px;box-shadow:0 18px 55px #44351e30}.hero{padding:27px 28px 22px;background:linear-gradient(135deg,#161613,#2d2415);color:#fff}.hero small{color:#f5ca72;letter-spacing:2px;font-size:10px}.hero h1{margin:8px 0 4px;font-family:Georgia,serif;font-size:29px;color:#efbc58}.hero p{margin:0;color:#d7c9ae;font-size:12px}.meta{display:flex;justify-content:space-between;padding:15px 28px;border-bottom:1px solid #eee5d5;font-size:12px;color:#6e6354}.meta b{color:#aa741d}.body{padding:18px 28px 28px}table{width:100%;border-collapse:collapse;font-size:12px}th{text-align:start;padding:8px 4px;color:#96826a;font-size:10px;text-transform:uppercase;border-bottom:1px solid #e9dfcf}th:not(:first-child),td:not(:first-child){text-align:end}td{padding:10px 4px;border-bottom:1px solid #f1eadf;vertical-align:top}td strong{display:block;font-size:12px}td small{display:block;margin-top:3px;color:#9d8c75;font-size:10px}.customer td{padding:16px 4px 7px;color:#a66d15;font-weight:700;border-bottom:0}.customer span{float:inline-end;color:#a59580;font-size:10px;font-weight:400}.totals{margin-top:18px;padding:14px 16px;border-radius:13px;background:#fbf4e6}.line{display:flex;justify-content:space-between;margin:8px 0;font-size:12px;color:#746552}.grand{padding-top:9px;border-top:1px dashed #dbc69b;color:#513b18;font-size:19px;font-weight:800}.footer{text-align:center;margin-top:20px;color:#9d8d77;font-size:10px;line-height:1.7}.print-action{display:block;width:calc(100% - 56px);margin:22px auto 0;padding:12px;border:0;border-radius:10px;background:#d99b2b;color:#201506;font-weight:700;cursor:pointer}@media print{body{padding:0;background:#fff}.receipt{box-shadow:none;border-radius:0;max-width:none}.print-action{display:none}}</style></head><body><main class="receipt"><header class="hero"><small>${branch || labels.cafeManagement}</small><h1>${cafeName}</h1><p>${contact || labels.preparedWithCare}</p></header><section class="meta"><span><b>${escapeHtml(receiptLabel)}</b><br/>${escapeHtml(orderLabel)}<br/>${labels.table} ${table} · ${orders[0].guests}</span><span>${issued.date}<br/>${issued.time}</span></section><section class="body"><table><thead><tr><th>${labels.item}</th><th>${labels.quantity}</th><th>${labels.price}</th><th>${labels.total}</th></tr></thead><tbody>${rows}</tbody></table><section class="totals"><div class="line"><strong>${labels.payment}</strong><strong style="color:${cash ? "#27834d" : "#b7791f"}">${cash ? labels.paidCash : labels.unpaidBill}</strong></div>${branding.receipt?.serviceEnabled ? `<div class="line"><span>${labels.service}${branding.receipt.serviceType === "percent" ? ` (${branding.receipt.serviceValue}%)` : ""}</span><span>${service.toFixed(2)} ${currency}</span></div>` : ""}${branding.receipt?.taxEnabled ? `<div class="line"><span>${labels.tax}${branding.receipt.taxType === "percent" ? ` (${branding.receipt.taxValue}%)` : ""}</span><span>${tax.toFixed(2)} ${currency}</span></div>` : ""}${cash ? `<div class="line"><span>${labels.cashReceived}</span><span>${cash.received.toFixed(2)} ${currency}</span></div><div class="line"><span>${labels.change}</span><span>${cash.change.toFixed(2)} ${currency}</span></div>` : ""}<div class="line grand"><span>${labels.total}</span><span>${total.toLocaleString()} ${currency}</span></div></section><footer class="footer">${footer}<br/>${cash ? paidReceiptNote : unpaidBillNote}</footer><button class="print-action" type="button" onclick="window.print()">▣ ${cash ? labels.printReceipt : labels.printBill}</button></section></main></body></html>`);
  bill.document.close();
