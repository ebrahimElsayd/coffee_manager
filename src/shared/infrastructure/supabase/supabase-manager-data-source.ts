...
import type { ManagerDataSource, ManagerOrder, ManagerSelectedOption, ManagerTable, WorkflowStatus, DrinkWorkflowStatus, PaymentStatus, PaymentMethod, PaymentReceipt, TableSessionStatus } from "@/shared/application/ports/manager-data-source";
...
export class SupabaseManagerDataSource implements ManagerDataSource {
 ...
 const db=getSupabaseBrowserClient();
  ...
  const [ir,pr]=await Promise.all([
   ids.length?db.from("order_items").select("id,order_id,product_id,product_name,recipient_name,quantity,unit_price,notes,selected_options,status,menu_products(image_url)").in("order_id",ids):Promise.resolve({data:[],error:null}),
    sessionIds.length?db.from("payments").select("session_id,method,received,received_amount,change_amount,status,paid_at,session_receipts(receipt_number,issued_at)").eq("status","paid").in("session_id",sessionIds).order("paid_at",{ascending:false}):Promise.resolve({data:[],error:null}),
  ]);
  ...
  return orders.map(o=>{const items=itemsByOrder.get(String(o.id))??[];const drinks=items.map((i:any)=>{const product=Array.isArray(i.menu_products)?i.menu_products[0]:i.menu_products;return {id:i.id,productId:i.product_id,name:i.product_name??"",imageUrl:product?.image_url??undefined,recipientName:i.recipient_name??"Guest",quantity:i.quantity,unitPrice:Number(i.unit_price||0),note:i.notes??"",selectedOptions:normalizeSelectedOptions(i.selected_options),status:drinkStatus[i.status]??"New"};});const people=[...new Set(drinks.map((d:any)=>d.recipientName).filter(Boolean))];const sessionPayments=payments.filter((p:any)=>p.session_id===o.session_id);const payment=sessionPayments.find((p:any)=>Array.isArray(p.session_receipts)?p.session_receipts.length>0:Boolean(p.session_receipts))??sessionPayments[0];const sessionHasPaidPayment=sessionPayments.some((p:any)=>p.status==="paid");const receipt=Array.isArray(payment?.session_receipts)?payment.session_receipts[0]:payment?.session_receipts;const received=payment?.received_amount ?? payment?.received;const session=Array.isArray(o.table_sessions)?o.table_sessions[0]:o.table_sessions;const table=Array.isArray(session?.cafe_tables)?session.cafe_tables[0]:session?.cafe_tables;return{id:String(o.id),sessionId:String(o.session_id),orderNumber:Number(o.order_number),table:table?.table_number==null?"":String(table.table_number),customer:people[0]??"Guest",time:new Date(o.created_at).toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"}),openedAt:o.created_at,guests:people.length?`${people.length} Guests`:"",status:orderStatus[o.status]??"New",drinks,orderType:"DineIn",paymentStatus:o.payment_status==="paid"||sessionHasPaidPayment?"Paid":"Unpaid",paymentMethod:payment?.method?String(payment.method).replace(/^./,(c:string)=>c.toUpperCase()) as PaymentMethod:undefined,cashReceived:received==null?undefined:Number(received),cashChange:payment?.change_amount==null?undefined:Number(payment.change_amount),receiptNumber:receipt?.receipt_number==null?undefined:Number(receipt.receipt_number),receiptIssuedAt:receipt?.issued_at??undefined,sessionStatus:session?.status==="closed"?"Closed":"Open",closedAt:session?.closed_at??o.closed_at??undefined} as ManagerOrder;});
 }
 ...
 async cancelDrink(orderId:string,id:string,reason:string){const db=getSupabaseBrowserClient();if(!db)throw new Error("Supabase unavailable");void orderId;const {error}=await db.rpc("manager_set_order_item_status",{p_order_item_id:id,p_status:"cancelled",p_note:reason||null});if(error)throw error;}
  async updateTablePayment(table:string,status:PaymentStatus,method:PaymentMethod,receivedAmount?:number,changeAmount?:number,sessionId?:string):Promise<PaymentReceipt|undefined>{const db=getSupabaseBrowserClient();if(!db)throw new Error("Supabase unavailable");void method;void changeAmount;if(status!=="Paid")return;const cafeId=await getManagerCafeId();let sessionQuery=db.from("table_sessions").select("id,cafe_tables!inner(cafe_id,table_number)").eq("cafe_tables.cafe_id",cafeId).eq("cafe_tables.table_number",Number(table)).in("status",["open","ordering","payment_pending"]);if(sessionId)sessionQuery=sessionQuery.eq("id",sessionId);else sessionQuery=sessionQuery.order("started_at",{ascending:false}).limit(1);const session=await sessionQuery.maybeSingle();if(session.error)throw session.error;if(!session.data)throw new Error("No active table session");let applyTax=false;let applyService=false;try{const raw=window.localStorage.getItem("kings-cafe-manager-settings");const parsed=raw?JSON.parse(raw) as {receipt?:{taxEnabled?:unknown;serviceEnabled?:unknown}}:null;applyTax=parsed?.receipt?.taxEnabled===true;applyService=parsed?.receipt?.serviceEnabled===true;}catch{}const {data,error}=await withTimeout(db.rpc("manager_record_cash_payment",{p_session_id:session.data.id,p_apply_tax:applyTax,p_apply_service:applyService,p_received_amount:receivedAmount ?? null}),15_000);if(error){console.error("[Payments] manager_record_cash_payment rejected",{message:error.message,details:error.details,hint:error.hint,code:error.code,sessionId:session.data.id});throw error;}const value=data as Record<string,unknown>;return{paymentId:String(value.payment_id),sessionId:String(value.session_id),receiptNumber:Number(value.receipt_number),receiptIssuedAt:String(value.receipt_issued_at),subtotal:Number(value.subtotal),serviceAmount:Number(value.service_amount),taxAmount:Number(value.tax_amount),grandTotal:Number(value.grand_total),receivedAmount:value.received_amount==null?undefined:Number(value.received_amount),changeAmount:Number(value.change_amount),reused:value.reused===true};}
