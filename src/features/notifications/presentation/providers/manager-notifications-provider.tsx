    ...
    <NotificationContext.Provider value={value}>
      ...
      {activeServiceRequest && <section className="pointer-events-none fixed inset-x-4 top-4 z-[125] flex justify-center" aria-live="assertive" aria-label="Pending table service request">
        <article className="pointer-events-auto w-full max-w-xl rounded-2xl border border-[var(--gold)]/70 bg-[#21190b]/[.98] p-4 shadow-[0_22px_70px_rgba(0,0,0,.6)] backdrop-blur-xl sm:p-5">
          ...
          <div className="mt-4 flex flex-wrap justify-end gap-2"><button type="button" onClick={() => { void acknowledgeRequest(activeServiceRequest.id).finally(() => router.push(`/orders?table=${encodeURIComponent(activeServiceRequest.table)}`)); }} className="rounded-lg border border-white/20 px-3 py-2 text-xs text-white/75 transition hover:bg-white/10">Open table</button><button type="button" onClick={() => { void acknowledgeRequest(activeServiceRequest.id); }} className="rounded-lg bg-[var(--gold)] px-3 py-2 text-xs font-semibold text-[#17120a] transition hover:brightness-110">Acknowledge</button></div>
        </article>
      </section>}
