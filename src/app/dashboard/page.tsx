...
function formatSessionOpenedAt(value: string | undefined, locale: "ar" | "en") {
  if (!value) return "—";
...
export default function DashboardPage() {
  ...
  const { pick, locale } = useManagerI18n();
        ...
        .on("postgres_changes", { event: "UPDATE", schema: "public", table: "cafe_tables", filter: `cafe_id=eq.${cafeId}` }, (payload) => {
          ...
          if (typeof row.table_number === "number") {
            setTables((current) => current.map((table) => table.number !== String(row.table_number) ? table : {
              ...table,
              status: row.status === "available" ? "Available" : "Seated",
  ...
  const unpaidBillOrders = activeBillOrders.filter((order) => order.paymentStatus !== "Paid");
  const isPaid = activeBillOrders.length > 0 && unpaidBillOrders.length === 0;
  const billTotals = calculateBillTotals(isPaid ? activeBillOrders : unpaidBillOrders, settings.receipt);
  ...
  const confirmPayment = async (receivedAmount: number, shouldPrintReceipt: boolean) => {
    ...
    try {
      const receipt = await managerWorkflow.markTablePaid(selectedTable, "Cash", receivedAmount, changeAmount, selectedOrder?.sessionId);
      if (shouldPrintReceipt && receipt) printBill(selectedTable, unpaidBillOrders, { ...settings, locale, footer: settings.receipt.footer, receipt: settings.receipt }, { received: receipt.receivedAmount ?? receivedAmount, change: receipt.changeAmount }, receiptWindow, true, { number: receipt.receiptNumber, issuedAt: receipt.receiptIssuedAt });
      closePayment();
      if (selectedOrder?.orderType === "Takeaway") await managerWorkflow.closeTableSession(selectedTable, selectedOrder.sessionId);
      await refresh(true);
  ...
  const closePaidTable = async () => {
    ...
    await managerWorkflow.closeTableSession(selectedTable, selectedOrder.sessionId);
  ...
  const closeEmptySeatedTable = async () => {
    const table = tables.find((item) => item.number === selectedTable);
    if (table?.status !== "Seated" || !table.sessionId || tableOrders.length) return;
    await managerWorkflow.closeTableSession(selectedTable, table.sessionId);
    setTables((current) => current.map((item) => item.number !== selectedTable ? item : { ...item, status: "Available", sessionId: undefined, sessionStatus: "Closed", guests: "" }));
    await refresh(true);
    notify(`Table ${selectedTable} closed and is now available`);
  };
  ...
  return <main className="manager-page min-h-screen overflow-x-hidden bg-[#080a09] pl-16 text-[#f4efe5] lg:pl-[184px] xl:pl-[200px] 2xl:pl-[224px]">
    ...
    <div className="dashboard-shell min-h-screen p-3 sm:p-5 xl:p-7">
      ...
      <section className="mt-4 grid gap-4 xl:grid-cols-[250px_minmax(0,1fr)_280px]" dir="ltr">
        ...
        <section className="min-w-0 rounded-xl border border-white/10 bg-[#101211] p-4"><header className="flex flex-wrap items-start justify-between gap-3 border-b border-white/10 pb-4"><div><h2 className="text-xl font-bold">Table {selectedTable}</h2><p className="mt-2 text-xs text-white/55">◷ {pick("Opened", "فتح الطاولة")}: {formatSessionOpenedAt(selectedOrder?.openedAt ?? tables.find((table) => table.number === selectedTable)?.openedAt ?? selectedOrder?.time, locale)}　•　♙ {pick("Cashier", "الكاشير")}: {selectedOrder?.cashierName ?? "—"}</p></div><div className="session-actions"><StatusBadge status={tableStatus}/>{isPaid ? <button type="button" title="Reprint the paid receipt" onClick={() => printBill(selectedTable, tableOrders, { ...settings, locale, footer: settings.receipt.footer, receipt: settings.receipt }, selectedOrder?.cashReceived !== undefined ? { received: selectedOrder.cashReceived, change: selectedOrder.cashChange ?? 0 } : undefined, undefined, true, selectedOrder?.receiptNumber && selectedOrder.receiptIssuedAt ? { number: selectedOrder.receiptNumber, issuedAt: selectedOrder.receiptIssuedAt } : undefined)} className="action-button bill-action">▣ <span>إعادة طباعة الإيصال</span></button> : null}{tableStatus === "Seated" && !tableOrders.length ? <button type="button" onClick={() => void closeEmptySeatedTable()} className="action-button bill-action">○ <span>{pick("Customer left · Close table", "غادر العميل · إغلاق الطاولة")}</span></button> : null}<button type="button" onClick={() => notify(`Alert sent to Table ${selectedTable}`)} className="action-button alert-action">♧ <span>Alert Table</span></button></div></header>
          ...
          <footer className="mt-4 grid gap-4 border-t border-white/10 pt-4 md:grid-cols-2"><div className="flex items-center gap-4"><span className="grid size-16 place-items-center rounded-xl border border-[var(--gold)]/60 bg-[var(--gold)]/10 text-[var(--gold)]"><svg viewBox="0 0 48 48" className="size-10" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="10" y="5" width="28" height="38" rx="3"/><path d="M16 13h16M16 21h16M16 29h5M27 29h5M16 36h5M27 36h5"/></svg></span><div><p className="text-sm text-white/70">Combined Table Total</p><strong className="text-4xl">{billTotals.total.toLocaleString()} <small className="text-lg font-medium text-white/55">{currency}</small></strong><p className="text-xs text-white/45">إجمالي طلب الطاولة · {totalItems} items</p></div></div><div><label className="mb-2 block text-sm">Payment</label><div className={`flex w-full items-center justify-between rounded-lg border px-3 py-2.5 text-sm ${isPaid ? "border-emerald-400/30 bg-emerald-400/10 text-emerald-300" : "border-white/20 text-white/55"}`}><span>{isPaid ? pick(`Paid · waiting for customer to leave`, "تم الدفع · بانتظار مغادرة العميل") : tableStatus === "Delivered" ? "Ready for payment" : "Available after delivery"}</span><span>{isPaid ? "✓" : "▣"}</span></div>{isPaid && selectedOrder?.orderType === "DineIn" ? <button type="button" onClick={() => void closePaidTable()} className="mt-2 w-full rounded-lg bg-[#e5aa3a] py-2.5 text-sm font-bold text-[#17120a]">○ {pick("Customer left · Close table", "غادر العميل · إغلاق الطاولة")}</button> : <button type="button" disabled={!canPay} onClick={() => setPaymentOpen(true)} className="mt-2 w-full rounded-lg bg-[#e5aa3a] py-2.5 text-sm font-bold text-[#17120a] disabled:cursor-not-allowed disabled:opacity-40">▣　{isPaid ? "Payment completed" : "Collect Cash"}</button>}</div></footer>
        </section>
      ...
      {paymentOpen && <PaymentDialog key={`${selectedTable}-cash`} open reference={`Table ${selectedTable}`} orderType={selectedOrder?.orderType ?? "DineIn"} orders={unpaidBillOrders} currency={currency} {...billTotals} onCancel={closePayment} onPrintBill={() => printBill(selectedTable, unpaidBillOrders, { ...settings, locale, footer: settings.receipt.footer, receipt: settings.receipt })} onConfirm={confirmPayment} />}
    </div>
  </main>;
}
