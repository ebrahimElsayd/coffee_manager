...
export function ManagerSidebar() {
  ...
  return (
    <aside className="manager-sidebar fixed inset-y-0 left-0 z-20 flex w-16 flex-col border-r border-white/[.08] bg-[#0b0d0d] px-1.5 py-4 text-[#e8e2d6] shadow-2xl lg:w-[184px] lg:px-2 xl:w-[200px] xl:px-2.5 2xl:w-[224px] 2xl:px-3 2xl:py-6">
      ...
      <AccountSettingsDialog open={accountSettingsOpen} settings={{ ...settings, adminEmail: identity?.email || "", adminPhone: identity?.phone || "" }} role={identity?.role} emailConfirmed={identity?.emailConfirmed} phoneConfirmed={identity?.phoneConfirmed} onClose={() => setAccountSettingsOpen(false)} onSavePhone={async (phone) => { const db = getSupabaseBrowserClient(); if (db) { const { error } = await db.auth.updateUser({ phone }); if (error) throw error; return { verificationRequired: true }; } updateSettings({ adminPhone: phone }); return { verificationRequired: false }; }} onVerifyPhone={async (phone, token) => { const db = getSupabaseBrowserClient(); if (!db) return; const { error } = await db.auth.verifyOtp({ phone, token, type: "phone_change" }); if (error) throw error; }} onPhoneVerified={(phone) => updateSettings({ adminPhone: phone })} onRequestPasswordReset={async () => { const db = getSupabaseBrowserClient(); if (!db || !identity?.email) throw new Error("Account email is unavailable."); const { error } = await db.auth.resetPasswordForEmail(identity.email, { redirectTo: `${window.location.origin}/reset-password` }); if (error) throw error; }} />
...
function Icon({ name, active }: { name: string; active: boolean }) {
  ...
  if (name === "products") return <svg className="size-7 shrink-0" viewBox="0 0 24 24" aria-hidden="true"><rect x="5" y="3.5" width="14" height="17" rx="2" {...common} /><path d="M9 3.5h6v3H9zM9 10h6M9 14h6M9 18h3" {...common} /><circle cx="7.5" cy="10" r=".35" fill={stroke} stroke="none" /><circle cx="7.5" cy="14" r=".35" fill={stroke} stroke="none" /><circle cx="7.5" cy="18" r=".35" fill={stroke} stroke="none" /></svg>;
